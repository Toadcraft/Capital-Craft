...
public class IronRankUnlockProcedure {
	public static void execute(Entity entity) {
		if (entity == null)
			return;
		if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Argent >= 50 && entity instanceof ServerPlayer _plr0 && _plr0.level() instanceof ServerLevel _serverLevel0
				&& _plr0.getAdvancements().getOrStartProgress(_serverLevel0.getServer().getAdvancements().get(ResourceLocation.parse("minecraft:story/mine_diamond"))).isDone() && entity instanceof ServerPlayer _plr1
				&& _plr1.level() instanceof ServerLevel _serverLevel1 && _plr1.getAdvancements().getOrStartProgress(_serverLevel1.getServer().getAdvancements().get(ResourceLocation.parse("minecraft:story/enter_the_nether"))).isDone()
				&& entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Rank == 1) {
			{
			...
			if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Rank >= 2) {
				...
				if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Argent < 50) {
					if (entity instanceof Player _player && !_player.level().isClientSide())
					...
					if (!(entity instanceof ServerPlayer _plr7 && _plr7.level() instanceof ServerLevel _serverLevel7
							&& _plr7.getAdvancements().getOrStartProgress(_serverLevel7.getServer().getAdvancements().get(ResourceLocation.parse("minecraft:story/mine_diamond"))).isDone())) {
						if (entity instanceof Player _player && !_player.level().isClientSide())
