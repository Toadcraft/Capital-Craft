...
public class Competense03Procedure {
	public static void execute(Entity entity) {
		if (entity == null)
			return;
		if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).skills_points >= 2 && entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).skill_03 == 0 && entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).skill_01 == 1) {
			{
			...
			if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).skill_03 == 1) {
				if (entity instanceof Player _player && !_player.level().isClientSide())
					_player.displayClientMessage(Component.literal("VOUS AVEZ DEJA CE SKILL"), false);
				if (entity instanceof Player _player)
					_player.closeContainer();
			} else if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).skill_01 == 0) {
				if (entity instanceof Player _player && !_player.level().isClientSide())
