...
public class GrainecobaltProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if ((world.getBlockState(BlockPos.containing(x, y, z))) == CapitalModeModBlocks.DOLOMITE_FARMLAND.get().defaultBlockState() && ((world.getBlockState(BlockPos.containing(x, y + 1, z))) == Blocks.AIR.defaultBlockState()
			...
			if (getEntityGameType(entity) == GameType.CREATIVE) {
				world.setBlock(BlockPos.containing(x, y + 1, z), CapitalModeModBlocks.COBALTPLANT.get().defaultBlockState(), 3);
			} else {
				if (entity instanceof Player _player) {
					ItemStack _stktoremove = new ItemStack(CapitalModeModItems.COBALTSEED.get());
