...
public class NetheriteRankUnlockProcedure {
	public static void execute(Entity entity) {
		if (entity == null)
			return;
		if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Argent >= 400 && entity instanceof ServerPlayer _plr0 && _plr0.level() instanceof ServerLevel _serverLevel0
				&& _plr0.getAdvancements().getOrStartProgress(_serverLevel0.getServer().getAdvancements().get(ResourceLocation.parse("capital_mode:leblocdetrop"))).isDone() && entity instanceof ServerPlayer _plr1
				&& _plr1.level() instanceof ServerLevel _serverLevel1 && _plr1.getAdvancements().getOrStartProgress(_serverLevel1.getServer().getAdvancements().get(ResourceLocation.parse("minecraft:nether/netherite_armor"))).isDone()
				&& entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Rank == 4) {
			{
			...
			if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Rank >= 5) {
				...
				if (entity.getData(CapitalModeModVariables.PLAYER_VARIABLES).Argent < 400) {
					if (entity instanceof Player _player && !_player.level().isClientSide())
					...
					if (!(entity instanceof ServerPlayer _plr7 && _plr7.level() instanceof ServerLevel _serverLevel7
						...
						if (!(entity instanceof ServerPlayer _plr10 && _plr10.level() instanceof ServerLevel _serverLevel10
								&& _plr10.getAdvancements().getOrStartProgress(_serverLevel10.getServer().getAdvancements().get(ResourceLocation.parse("minecraft:nether/netherite_armor"))).isDone())) {
							if (entity instanceof Player _player && !_player.level().isClientSide())
