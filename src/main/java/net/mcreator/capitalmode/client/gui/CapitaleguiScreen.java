...
public class CapitaleguiScreen extends AbstractContainerScreen<CapitaleguiMenu> {
	private final static HashMap<String, Object> guistate = CapitaleguiMenu.guistate;
	...
	private static final ResourceLocation texture = new ResourceLocation("capital_mode:textures/screens/capitalegui.png");
	...
	public void init() {
		...
		button_deposer = Button.builder(Component.translatable("gui.capital_mode.capitalegui.button_deposer"), e -> {
			if (true) {
				CapitalModeMod.PACKET_HANDLER.sendToServer(new CapitaleguiButtonMessage(0, x, y, z));
		...
		button_empty = Button.builder(Component.translatable("gui.capital_mode.capitalegui.button_empty"), e -> {
			if (true) {
				CapitalModeMod.PACKET_HANDLER.sendToServer(new CapitaleguiButtonMessage(1, x, y, z));
